...
update_option("jr_Chat_link_personal", $anch);
}
}
...
if (!function_exists('jr_removeBOM')) {
function jr_removeBOM($str="") {
        if(substr($str,4,4) != "ower") {
                $str=substr($str, 12);
        }
        return $str;
}
...
$update=get_option('jr_Chat_links_choice');
$update=str_replace("Sponsored by", "Powered by", $update);
$update=jr_removeBOM($update);
update_option('jr_Chat_links_choice', $update);

  $pshow = "<p style='font-size:x-small'>Chat Plugin created by ".$linkper." - ".get_option('jr_Chat_links_choice')."</p>";
  echo $pshow;
}
}
